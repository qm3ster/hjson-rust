    ...
    Iter: Iterator<Item = u8>,
{
    ...
    pub fn end(&mut self) -> Result<()> {
        r#try!(self.rdr.parse_whitespace());
        if r#try!(self.rdr.eof()) {
            Ok(())
        } else {
        ...
        V: de::Visitor,
    {
        ...
        loop {
            let ch = r#try!(self.rdr.next_char_or_null());
        ...
        V: de::Visitor,
    {
        ...
        let value = match r#try!(self.rdr.peek_or_null()) {
            /*
            b'-' => {
                self.rdr.eat_char();
    ...
    fn parse_ident(&mut self, ident: &[u8]) -> Result<()> {
        for c in ident {
            if Some(*c) != r#try!(self.rdr.next_char()) {
        ...
        V: de::Visitor,
    {
        ...
        let first = r#try!(self.rdr.peek()).unwrap();
        ...
        loop {
            ...
            let is_comment = ch == b'#'
                || if ch == b'/' {
                    let next = r#try!(self.rdr.peek_or_null());
                    next == b'/' || next == b'*'
                } else {
                    false
                };
    ...
    fn decode_hex_escape(&mut self) -> Result<u16> {
        let mut i = 0;
        let mut n = 0u16;
        while i < 4 && !r#try!(self.rdr.eof()) {
            n = match r#try!(self.rdr.next_char_or_null()) {
    ...
    fn ml_skip_white(&mut self) -> Result<bool> {
        match r#try!(self.rdr.peek_or_null()) {
            b' ' | b'\t' | b'\r' => {
                self.rdr.eat_char();
                Ok(true)
            }
    ...
    fn ml_skip_indent(&mut self, indent: usize) -> Result<()> {
        let mut skip = indent;
        while r#try!(self.ml_skip_white()) && skip > 0 {
            skip -= 1;
        }
        Ok(())
    }
        ...
        V: de::Visitor,
    {
        ...
        while r#try!(self.ml_skip_white()) {}
        if r#try!(self.rdr.peek_or_null()) == b'\n' {
            self.rdr.eat_char();
            r#try!(self.ml_skip_indent(indent));
        }
    ...
    fn parse_string(&mut self) -> Result<()> {
        ...
        loop {
            let ch = match r#try!(self.rdr.next_char()) {
                Some(ch) => ch,
                None => {
                    return Err(self.rdr.error(ErrorCode::EOFWhileParsingString));
                }
            ...
            match ch {
                ...
                b'\\' => {
                    ...
                    match ch {
                        ...
                        b'u' => {
                            let c = match r#try!(self.decode_hex_escape()) {
                                0xDC00..=0xDFFF => {
                                    return Err(self
                                        .rdr
                                ...
                                n1 @ 0xD800..=0xDBFF => {
                                    match (r#try!(self.rdr.next_char()), r#try!(self.rdr.next_char())) {
                                        (Some(b'\\'), Some(b'u')) => (),
                                        _ => {
                                            return Err(self
                                                .rdr
                                    ...
                                    let n2 = r#try!(self.decode_hex_escape());
    ...
    fn parse_object_colon(&mut self) -> Result<()> {
        ...
        match r#try!(self.rdr.next_char()) {
            Some(b':') => Ok(()),
...
struct SeqVisitor<'a, Iter: Iterator<Item = u8>> {
    ...
    Iter: Iterator<Item = u8>,
{
    ...
    where
        T: de::Deserialize,
    {
        r#try!(self.de.rdr.parse_whitespace());

        match r#try!(self.de.rdr.peek()) {
            Some(b']') => {
                return Ok(None);
            }
            Some(_) => {}
        ...
        let value = r#try!(de::Deserialize::deserialize(self.de));

        // in Hjson the comma is optional and trailing commas are allowed
        r#try!(self.de.rdr.parse_whitespace());
        if r#try!(self.de.rdr.peek()) == Some(b',') {
    ...
    fn end(&mut self) -> Result<()> {
        ...
        match r#try!(self.de.rdr.next_char()) {
            Some(b']') => Ok(()),
...
struct MapVisitor<'a, Iter: Iterator<Item = u8>> {
    de: &'a mut Deserializer<Iter>,
    first: bool,
    root: bool,
}
    ...
    Iter: Iterator<Item = u8>,
{
        ...
        K: de::Deserialize,
    {
        ...
        if self.first {
            self.first = false;
        } else if r#try!(self.de.rdr.peek()) == Some(b',') {
            // in Hjson the comma is optional and trailing commas are allowed
            self.de.rdr.eat_char();
            r#try!(self.de.rdr.parse_whitespace());
        }
    ...
    where
        V: de::Deserialize,
    {
        r#try!(self.de.parse_object_colon());
    ...
    Iter: Iterator<Item = u8>,
{
    ...
    where
        V: de::Deserialize,
    {
        let val = r#try!(de::Deserialize::deserialize(self));
        r#try!(self.parse_object_colon());
        Ok(val)
    }
